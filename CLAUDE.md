...
- **`UrlGenerator`** — reverse: builds a URL from a controller + element (id or row). Placeholder strings like `//rel0//` are inserted for relationship fields and resolved in a second pass via the resolver. Has an in-instance row cache keyed by entity + id. `urlEncode` handles Unicode (Cyrillic, Han) and replaces any char outside `[a-z0-9а-я\p{Han}_-]` with a dash (not strips), then collapses repeats — this keeps it aligned with `UrlMatcher`, which splits URL segments on `-` and turns them into `%` LIKE wildcards, so punctuation like `'` and `&` must survive as separators on both sides.
